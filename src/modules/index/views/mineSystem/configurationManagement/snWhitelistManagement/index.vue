<template>
    <div class="sn-mine--warp">
        <div class="title-mip">
            <con-header :title-icon="addProductsIcon" title="SN白名单管理" :hasBottomBorder="true"></con-header>
        </div>
        <!-- 查询部分 -->
        <div class="mip-check">
           <el-form :inline="true" ref="formInline" :model="formInline" class="demo-form-inline" label-width="180px" :rules="rules">
              <el-row>
                <el-col :span="12">
...
import { snWhitelistManagementMixin } from '@/modules/index/views/mineSystem/mixin';
export default {
  components: {
    conHeader,
    minePagination
  },
  mixins: [ snWhitelistManagementMixin ],
  data () {
    return {
      rules: {
        mobile: { required: false, validator: this.checkMobile, trigger: 'change' },
        sn: [{ required: false, validator: this.checkSn, trigger: 'change' }]
      },
      loading: false,
